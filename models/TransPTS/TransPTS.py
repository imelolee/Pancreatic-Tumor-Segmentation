...
from models.TransPTS.Transformer import TransformerModel
from models.TransPTS.PositionalEncoding import FixedPositionalEncoding,LearnedPositionalEncoding
from models.TransPTS.Unet_skipconnection import Unet


class TransformerPTS(nn.Module):
    def __init__(
        self,
        img_dim,
        patch_dim,
        ...
        super(TransformerPTS, self).__init__()
    ...
    def encode(self, x):
        if self.conv_patch_representation:
            ...
            x = x.view(x.size(0), -1, self.embedding_dim) # [4, 1024, 512]
        ...
        x = self.position_encoding(x) # [4, 1024, 512]
        x = self.pe_dropout(x)
    ...
    def forward(self, x, img_shape, auxillary_output_layers=[1, 2, 3, 4]):

        x1_1, x2_1, x3_1, encoder_output, intmd_encoder_outputs = self.encode(x)

        decoder_output = self.decode(
            x1_1, x2_1, x3_1, encoder_output, intmd_encoder_outputs, img_shape, auxillary_output_layers
        )
    ...
    def _reshape_output(self, x, img_shape):
        x = x.view(x.size(0), int(img_shape[0] / self.patch_dim), int(img_shape[1] / self.patch_dim),
                   int(img_shape[2] / self.patch_dim), self.embedding_dim, )
...
class PTS(TransformerPTS):
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, img_shape, intmd_layers=[1, 2, 3, 4]):
        ...
        x8 = self._reshape_output(x8, img_shape)
